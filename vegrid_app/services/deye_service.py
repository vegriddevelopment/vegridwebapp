            ...
            for point in history_data.get('dataList', []):
                ...
                for val in point.get('itemList', []):
                    ...
                    if period == 'day':
                        ...
                        if key == 'TotalDCInputPower': item['generation'] = energy_val
                        elif key == 'TotalConsumptionPower': item['consumption'] = energy_val
                        elif key == 'BatteryPower': item['storage'] = abs(energy_val) 
                        elif key == 'TotalGridPower': item['grid'] = abs(energy_val)
                    elif granularity in [2, 3]:
                        # Daily/Monthly totals from granularity 2 or 3
                        # We use multiple keys here to handle different inverter models/API versions
                        if key in ['Production', 'yield']: item['generation'] = value
                        elif key in ['Consumption', 'load']: item['consumption'] = value
                        elif key in ['DischargingCapacity', 'batteryDischarge']: item['storage'] = value
                        elif key in ['ElectricityPurchasing', 'gridImport']: item['grid'] = value
                    else:
                        ...
                        elif key == 'DailyDischargingEnergy': item['storage'] = value
                        elif key == 'DailyEnergyPurchased': item['grid'] = value
