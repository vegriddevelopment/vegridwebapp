...
def device_data(request, device_sn):
    """AJAX endpoint to fetch real-time data for a device (station) or all devices"""
    try:
        customer = request.user.customer
        service = DeyeService()
        ...
        if device_sn == 'all':
            ...
            for device in devices:
                data = service.get_station_detail(device.device_sn)
                if data:
                    total_data['pvPower'] += data.get('pvPower', 0)
                    total_data['gridPower'] += data.get('gridPower', 0)
                    total_data['batteryPower'] += data.get('batteryPower', 0)
                    total_data['loadPower'] += data.get('loadPower', 0)
                    
                    soc = data.get('batterySoc', 0)
                    if soc > 0:
                        soc_sum += soc
                        soc_count += 1
                        
                    total_data['generationToday'] += data.get('dailyProduction', 0)
                    total_data['consumptionToday'] += data.get('dailyConsumption', 0)
                    total_data['batteryDischargeToday'] += data.get('dailyDischarge', 0)
                    total_data['gridToday'] += data.get('dailyGridImport', 0)
                    total_data['pvSelfConsumption'] += data.get('pvSelfConsumption', 0)
            
            if soc_count > 0:
                total_data['batterySoc'] = soc_sum / soc_count
...
def filtered_data(request, station_id):
    ...
    try:
        ...
        devices = []
        if station_id == 'all':
            devices = list(DeyeDevice.objects.filter(customer=customer))
            if not devices:
                # Try sync if no devices
                service.sync_site_names(customer=customer)
                devices = list(DeyeDevice.objects.filter(customer=customer))
        else:
            try:
        ...
        for idx, device in enumerate(devices):
            real_station_id = service.get_station_id_by_device_sn(device.device_sn) or device.device_sn
            
            # Fetch data based on period
            if period == 'day':
                d_data = service.get_station_energy_day(real_station_id, timezone.now().strftime('%Y-%m-%d'), device_sn=device.device_sn)
            elif period == 'month':
                d_data = service.get_station_energy_month(real_station_id, timezone.now().strftime('%Y-%m'), device_sn=device.device_sn)
            elif period == 'year':
                d_data = service.get_station_energy_year(real_station_id, str(timezone.now().year), device_sn=device.device_sn)
            elif period == 'lifetime':
                d_data = service.get_station_energy_lifetime(real_station_id, device_sn=device.device_sn)
            else:
                continue

            if d_data.get('code') in [0, "0", "1000000"]:
                items = d_data.get('data', {}).get('items', [])
                if not all_items:
                    all_items = items
                else:
                    # Merge items by time
                    for item in items:
                        matching = next((x for x in all_items if x['time'] == item['time']), None)
                        if matching:
                            matching['generation'] = (matching.get('generation') or 0) + (item.get('generation') or 0)
                            matching['pv_production'] = (matching.get('pv_production') or 0) + (item.get('pv_production') or 0)
                            matching['consumption'] = (matching.get('consumption') or 0) + (item.get('consumption') or 0)
                            matching['storage'] = (matching.get('storage') or 0) + (item.get('storage') or 0)
                            matching['grid'] = (matching.get('grid') or 0) + (item.get('grid') or 0)
                        else:
                            all_items.append(item)
                
            # Fetch latest station statistics
            latest_resp = service.get_station_latest(real_station_id)
            if latest_resp.get('code') in [0, "0", "1000000"]:
                l_data = latest_resp.get('data', {})
                total_gen_today += float(l_data.get('generationToday', 0))
                total_gen_lifetime += float(l_data.get('generationTotal', 0))
                
                cons_today = float(l_data.get('consumptionToday', 0))
                total_consumption_today += cons_today
                
                # Formula: Solar = Consumption - Battery + Grid
                # Battery = Discharge - Charge, Grid = Export - Import
                # So Solar = Consumption - (Discharge - Charge) + (Export - Import)
                
                disch_today = float(l_data.get('batteryDischargeToday', 0))
                char_today = float(l_data.get('batteryChargeToday', 0))
                imp_today = float(l_data.get('gridToday', 0)) # gridToday usually means Grid Import
                exp_today = float(l_data.get('gridExportToday', 0))
                
                total_discharge_today += disch_today
                total_grid_today += imp_today
                
                # Calculate Solar for this device using the formula
                calc_solar_today = cons_today - (disch_today - char_today) + (exp_today - imp_today)
                if calc_solar_today < 0: calc_solar_today = 0
                
                total_solar_today += calc_solar_today
            
            # Use first device for weather/location if not 'all'
            if station_id != 'all' and idx == 0:
                ...
                if detail_resp.get('code') in [0, "0", "1000000"]:
                    ...
                    station_info = {
                        'location': location,
                        'weather': f"{temp}°C {weather_name}" if temp is not None else weather_name,
                        'timezone': detail_data.get('timezone', '3'),
                        'current_time': timezone.now().strftime('%H:%M:%S')
                    }
        ...
        return JsonResponse({
            ...
            'station_info': station_info,
            'generationToday': total_gen_today,
            'generationTotal': total_gen_lifetime,
        ...
        logger.error(f"Error in filtered_data: {e}")
        return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
...
def send_sms_otp(phone_number, otp_code):
    ...
    if username and api_key:
        try:
            ...
            africastalking.initialize(username, api_key)
            sms = africastalking.SMS
            # Clean phone number - ensure it's in correct format (e.g., +254700000000)
            clean_phone = phone_number.strip().replace(' ', '')
            if not clean_phone.startswith('+'):
                clean_phone = '+' + clean_phone
    ...
    print(f"WARNING: SMS sending failed. OTP for {phone_number} is: {otp_code}")
...
class SendPhoneOtpView(View):
    ...
    def post(self, request):
        ...
        # Create or get user (temporary user until details are provided)
